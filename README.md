...
By default, the site runs in **static mode** — it shows benchmark data from the protocol's v0.1.68+ single-node baselines. A **public testnet node** is live at `https://78.47.43.136.sslip.io`. Note what that node actually is today: **a single node with zero peers**, reporting `not_ready` on `/readyz`. Multi-node Lane 0 BFT finality (10k-event bursts, 3-region WAN) was measured in stress runs — see the protocol repo's `docs/reference/benchmark-gates.md` — but no standing validator mesh is running. Don't describe it as one on the site. To connect the site to the node (or any node):
