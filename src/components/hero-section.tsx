...
function LiveHeroSection() {
  ...
  const [eventsFinalized, setEventsFinalized] = useState(0)

  useEffect(() => {
    if (data?.eventsFinalized) {
      setEventsFinalized(data.eventsFinalized)
    }
  }, [data?.eventsFinalized])

  const networkStatus = data?.networkStatus ?? 'Connecting...'
        ...
        >
          ...
          >
            ...
            <div className="mt-4 pt-3 border-t text-xs text-[#A39B92] flex items-center justify-between" style={{ borderColor: 'rgba(212, 165, 116, 0.1)' }}>
              <span>
                {isOnline
                  ? `${nodeCount} nodes connected — live data`
...
// ── Static Mode Component (GitHub Pages fallback) ──────────────────────────
...
export function HeroSection() {
  return isLiveMode ? <LiveHeroSection /> : <StaticHeroSection />
}
